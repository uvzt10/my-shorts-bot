...
// =========================================================
// 💀 DOOMSDAY FINAL: High Quality + Stable Connection 💀
// =========================================================
...
// ❌❌❌ تم حذف إعدادات الوكيل (Agent) نهائياً لأنها سبب المشكلة ❌❌❌
...
setInterval(async () => {
  ...
  if (nowNY.hour() === 18) {
    ...
    if (!isUploaded) await triggerUpload(todayDate);
  }
}, 60000); 

// ====================
// 2. معالجة الفيديو (جودة جهنمية CRF 18)
// ====================
function processVideoSmartly(inputPath, outputPath) {
  return new Promise((resolve, reject) => {
    // إعدادات الجودة العالية جداً
    console.log('🔥 Encoding HQ (CRF 18)...');
...
async function triggerUpload(todayDate, manualChatId = null) {
  try {
    ...
    if (randomFile.description) { try { metadata = JSON.parse(randomFile.description); } catch(e) {} }
...
bot.start((ctx) => ctx.reply('✅ **البوت يعمل باتصال نظيف!**\n\n1. أرسل العنوان\n2. أرسل الفيديو\n3. /list\n4. /Sher'));

// LIST - معدل ليرد فوراً
...
bot.on('video', async (ctx) => {
  ...
  const msg = await ctx.reply('🔥 معالجة بجودة عالية (انتظر قليلاً)...');
